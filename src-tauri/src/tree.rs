...
const ILIAS_ROOT: &str = "ilias.php?baseClass=ilDashboardGUI&cmd=jumpToSelectedItems";
  ...
  pub async fn update_ilias(&self) -> Result<(), ClientError> {
    ...
    if let Some(credentials) = credentials {
      ...
      root_nodes
        ...
        .for_each(async |ilnode| {
          update_ilias_tree(self.client.clone(), ilnode, self.sender.clone())
            .await
            .unwrap();
          self.roots.push()
        });
          ...
          if let Some(children) = node.children.as_mut() {
            if sync {
              ...
              let new_children: Vec<Arc<Mutex<IlNode>>> = elements
                ...
                .map(|hypnode| {
                  if let Some(node_index) = children
                    .iter()
